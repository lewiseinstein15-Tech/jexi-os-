...
whenToUse: Use when the task matches this skill's scope: Standard single-cell RNA-seq analysis pipeline. Use for QC, normalization, dimensionality reduction (PCA/UMAP/t-SNE), clustering, differential expression, visualization, and converting R-friendly single-cell formats such as Seurat or SingleCellExperiment RDS files into h5ad for Scanpy. Best for exploratory scRNA-seq analysis with established workflows. For deep learning models use scvi-tools; for data format questions use anndata.
allowedTools: [Read, Write, Edit, Bash]
domain: genomics
tier: reference-only
origin: K-Dense-AI/scientific-agent-skills
upstreamPath: skills/scanpy/SKILL.md
...
- Source: K-Dense-AI/scientific-agent-skills `skills/scanpy/SKILL.md` @ `330c8e764435` (release 2.69.0); body ported verbatim.
...
- External scripts: NOT vendored — upstream ships 24 auxiliary file(s) (scripts/references/assets) for this skill; MIT permits copying, they stay upstream by import policy. Check provider ToS before operational use.
