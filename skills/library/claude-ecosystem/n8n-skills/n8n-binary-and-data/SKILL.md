...
whenToUse: "Handle files and binary data in n8n correctly. Use when working with files, images, PDFs, attachments, uploads or downloads, base64, vision/multimodal input, or when an AI agent needs a file as tool input or output — and whenever the user mentions $binary, binaryPropertyName, \"read the PDF\", \"attach the file\", \"send the image\", Merge losing binary, or a CDN for chat images. Covers the $binary vs $json split, reading/writing binary, keeping binary alive across transforms with Merge, the agent-tool binary boundary, and the CDN/URL requirement for chat surfaces."
allowedTools: []
domain: claude-ecosystem
tier: reference-only
origin: czlonkowski/n8n-skills
upstreamPath: skills/n8n-binary-and-data/SKILL.md
...
## Import Provenance
- Source: czlonkowski/n8n-skills `skills/n8n-binary-and-data/SKILL.md` @ `19cd793f4789` (plugin release 1.35.0); body ported verbatim.
...
- External files: NOT vendored — upstream ships 5 auxiliary file(s) for this skill (AGENT_TOOL_BINARY.md, BINARY_BASICS.md, CDN_REQUIREMENT.md, MERGE_FOR_CONTEXT.md, README.md); MIT permits copying, they stay upstream by import policy.
