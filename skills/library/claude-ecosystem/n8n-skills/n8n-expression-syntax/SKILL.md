...
whenToUse: "Validate n8n expression syntax and fix common errors. Use when writing n8n expressions, using {{}} syntax, accessing $json/$node variables, troubleshooting expression errors, mapping data between nodes, or referencing webhook data in workflows. Use this skill whenever configuring node fields that reference data from previous nodes — expressions are how n8n passes data between nodes, and getting the syntax wrong is the most common source of workflow errors. Also use when asked whether a complex expression hurts performance."
allowedTools: []
domain: claude-ecosystem
tier: reference-only
origin: czlonkowski/n8n-skills
upstreamPath: skills/n8n-expression-syntax/SKILL.md
...
## Import Provenance
- Source: czlonkowski/n8n-skills `skills/n8n-expression-syntax/SKILL.md` @ `19cd793f4789` (plugin release 1.35.0); body ported verbatim.
...
- External files: NOT vendored — upstream ships 3 auxiliary file(s) for this skill (COMMON_MISTAKES.md, EXAMPLES.md, README.md); MIT permits copying, they stay upstream by import policy.
