...
export const TOOL_REGISTRY = [
  ...
  { slug: 'skill-execute', name: 'Skill Execute', type: 'Agent', desc: 'Run an executable skill: parse its ## Steps procedure and execute each step through the real tool registry (not a text prompt). Returns a structured result.', agents: ['planner', 'orchestrator', 'coder', 'engineer'], tier: 'exec', timeoutMs: 120000 },
  ...
  { slug: 'report', name: 'Subagent Report', type: 'Agent', desc: 'Deliver a self-contained result to the agent that started you. Only callable inside a subagent run; reporting never ends your turn (dsh tool-subagent-report).', agents: ['orchestrator', 'planner'], tier: 'read', timeoutMs: 15000 },
  { slug: 'schedule_create', name: 'Schedule Create', type: 'Schedule', desc: 'Create a recurring schedule: everySeconds (or dailyAt HH:MM) + a query to run (dsh schedule_create).', agents: ['task-manager', 'planner'], tier: 'write_local', timeoutMs: 15000 },
  { slug: 'pwsh', name: 'PowerShell', type: 'Execution', desc: 'Run a command in a PowerShell-dialect shell (pwsh -NoProfile). Native C:\ paths and $env:NAME variables. Fail-open when pwsh is not installed (dsh tool-pwsh).', agents: ['devops', 'coder'], tier: 'exec', timeoutMs: 120000 },
  { slug: 'cordis_inspect_list', name: 'Cordis Inspect List', type: 'System', desc: 'List every Cordis Inspect provider known to the host (plugins, tools, skills, services) with their read-only methods (dsh tool-cordis).', agents: ['jexi', 'orchestrator'], tier: 'read', timeoutMs: 15000 },
  { slug: 'cordis_define', name: 'Cordis Define', type: 'System', desc: 'Define a new dynamic Cordis plugin package: name, purpose, and host code body (dsh tool-cordis cordis_define).', agents: ['jexi', 'orchestrator'], tier: 'write_local', timeoutMs: 15000 },
  { slug: 'cordis_run', name: 'Cordis Run', type: 'System', desc: 'Start (or update) the active run of a defined package — its host code is evaluated against the live plugin seam (dsh tool-cordis cordis_run).', agents: ['jexi', 'orchestrator'], tier: 'exec', timeoutMs: 30000 },
  { slug: 'cordis_stop', name: 'Cordis Stop', type: 'System', desc: 'Stop a dynamic plugin’s active run (cleanup runs, registrations withdrawn) (dsh tool-cordis cordis_stop).', agents: ['jexi', 'orchestrator'], tier: 'write_local', timeoutMs: 15000 },
  { slug: 'cordis_undefine', name: 'Cordis Undefine', type: 'System', desc: 'Remove a dynamic plugin and all its packages (dsh tool-cordis cordis_undefine).', agents: ['jexi', 'orchestrator'], tier: 'write_local', timeoutMs: 15000 },
  { slug: 'cordis_inspect_self', name: 'Cordis Inspect Self', type: 'System', desc: 'Inspect the dynamic Cordis runner itself: plugins, packages, active runs (dsh tool-cordis cordis_inspect_self).', agents: ['jexi', 'orchestrator'], tier: 'read', timeoutMs: 15000 },
  { slug: 'cordis_inspect_query', name: 'Cordis Inspect Query', type: 'System', desc: 'Run a read-only inspect query: provider + method from cordis_inspect_list, input per the method schema. Cannot invoke business services or modify the runtime (dsh tool-cordis).', agents: ['jexi', 'orchestrator'], tier: 'read', timeoutMs: 15000 },
  { slug: 'schedule_list', name: 'Schedule List', type: 'Schedule', desc: 'List every schedule with cadence, status, and next run (dsh schedule_list).', agents: ['task-manager', 'planner'], tier: 'read', timeoutMs: 15000 },
  { slug: 'schedule_delete', name: 'Schedule Delete', type: 'Schedule', desc: 'Delete a schedule by its id (dsh schedule_delete).', agents: ['task-manager', 'planner'], tier: 'write_local', timeoutMs: 15000 },
