...
whenToUse: Use when the task matches this skill's scope: Read, validate, and safely export protocols.io data with current official REST/MCP contracts, or create non-executing mutation plans. The bundled client makes bounded official-host GET requests only with explicit --execute. Use only for tasks explicitly targeting protocols.io or an exact protocols.io protocol version.
allowedTools: [Read, Write, Python]
domain: lab-automation
tier: reference-only
origin: K-Dense-AI/scientific-agent-skills
upstreamPath: skills/protocolsio-integration/SKILL.md
...
- Source: K-Dense-AI/scientific-agent-skills `skills/protocolsio-integration/SKILL.md` @ `330c8e764435` (release 2.69.0); body ported verbatim.
