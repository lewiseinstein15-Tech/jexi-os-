...
| 12 | Orchestrator v2 (tool-calling loop, gated stop, retry policy) | ✅ shipped to main (AgentLoop: plan → generate → NATIVE tool_calls (B67, no JSON-in-prose) → ToolRuntime → results fed back → final answer; /api/agent NDJSON stream) |
...
| 18 | Computer runtime abstraction (provider-independent) | ✅ ComputerRuntime — local / remote (in-process bridge or VIRTUAL_API) / docker (declared) / mock providers, capability matrix, ComputerUseAgent dispatches through it, /api/computer/status + runtime line in Terminal |
| 19 | Browser agent improvements (screenshots, UI verification) | ✅ page snapshots (url/title/elements/text-hash), verifyChange after goto/click in the agent loop, persistent screenshots in DATA_DIR/screenshots + gallery endpoint, gallery strip in Terminal |
...
| 25 | Cloud/runtime deployment | ✅ DEPLOYMENT.md — Render blueprint verified (health endpoint), full env matrix incl. runtime vars (DATA_DIR persistence, PLAYWRIGHT_BROWSERS_PATH, COMPUTER_RUNTIME), frontend hosting flow; remaining ops work documented (persistent volume, REDIS_URL) |
...
| 29 | **BUILD 47 — intelligence layer** (context continuity, memory, autonomous orchestration) | ✅ shipped to main — TaskRegistry (persistent multi-task registry: create/switch/pause/resume/fail/retry, reference resolution with tie detection → clarify, resume-context blocks), ConversationManager (continue / switch / new / clarify classification, topic-switch detection via standalone-question heuristics, multi-task decomposition that never splits a list inside one objective), DecisionEngine (decides what to do, injects resume context so continue = a step not a restart, clarification path with candidate options), DecisionMemory (provenanced facts/decisions with subject-based conflict supersession — history kept, never invented), wired into `/api/chat` with `intel` NDJSON events + `/api/context` observability endpoints; frontend context chip in Command Center; 37-test acceptance suite incl. the unpredictable long-conversation simulation (research ↔ math ×2 with resumption) |
