...
whenToUse: Use when the task matches this skill's scope: Query and download public cancer imaging data from NCI Imaging Data Commons. Invoke for any question about IDC collections, cancer imaging datasets, DICOM data access, radiology (CT, MR, PET) or pathology AI training sets, metadata queries, visualization, or license checks — even when the user doesn't explicitly mention "IDC". No authentication required.
allowedTools: [Read, Write, Edit, Bash]
domain: medical-imaging
tier: reference-only
origin: K-Dense-AI/scientific-agent-skills
upstreamPath: skills/imaging-data-commons/SKILL.md
...
- Source: K-Dense-AI/scientific-agent-skills `skills/imaging-data-commons/SKILL.md` @ `330c8e764435` (release 2.69.0); body ported verbatim.
- License: MIT (upstream LICENSE.md, K-Dense Inc.); per-skill upstream license field: "This skill is provided under the MIT License. IDC data itself has individual licensing (mostly CC-BY, some CC-NC) that must be respected when using the data.".
