...
ok(pluginTools.length >= 1, `plugin tools mounted (${pluginTools.length}: ${pluginTools.map((t) => t.slug).join(', ')})`);
...
ok(TOOL_COUNT === 193, `registry count unchanged by plugin load (${TOOL_COUNT})`);
