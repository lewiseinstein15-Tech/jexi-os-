...
ok(pluginTools.length >= 5, `plugin tools mounted (${pluginTools.length}: ${pluginTools.map((t) => t.slug).join(', ')})`);
for (const slug of ['weather-now', 'time-now', 'currency-convert', 'crypto-price', 'ip-geo']) {
