...
| 33 | AndroidRuntime adapter (Part 13): real device computer-use via adb — terminal, browser (am start + uiautomator dump), input (tap/text/keyevent/swipe), screencap, push; honest COMPUTER_BLOCKED when adb/device absent | SHIPPED | `test-b225.js` (22 adapter checks via a stub adb binary: exact argv incl. `-s serial`, real XML→elements with bounds-center taps, adb escaping space→%s, PNG-magic validation, no-adb/no-device/offline honesty) + browser-round e2e on the adapter | Device-attached capabilities; the APK shell itself has no SpeechRecognition (mic button honestly absent in WebView) |
| 34 | Voice input: the browser is the microphone (Web Speech API — zero keys, zero server cost); discovery composes assignments (recommendation rides the brief) | SHIPPED | `test-b225.js` (Composer contracts: feature-detected render guard, interim results, honest error surfacing; `recommendedToolsForSubtask` units + Director wiring contracts) | Recommendation only — B52 allowlist + B209 gate still run at execution; mic absent where the engine is absent |
