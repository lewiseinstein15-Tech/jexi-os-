...
whenToUse: "Wire n8n error handling so failures are loud, structured, and recoverable. Use when building any webhook/API workflow, a scheduled or unattended workflow, or any path where a silent failure would drop user-visible work — and whenever the user mentions error handling, onError, continueErrorOutput, error branches/outputs, retries, retryOnFail, Respond to Webhook status codes, 4xx/5xx, Error Trigger, or \"my workflow fails silently\". Covers per-node error outputs and wiring, retry/self-healing, error-trigger workflows, and 4xx/5xx response shapes."
allowedTools: []
domain: claude-ecosystem
tier: reference-only
origin: czlonkowski/n8n-skills
upstreamPath: skills/n8n-error-handling/SKILL.md
...
## Import Provenance
- Source: czlonkowski/n8n-skills `skills/n8n-error-handling/SKILL.md` @ `19cd793f4789` (plugin release 1.35.0); body ported verbatim.
...
- External files: NOT vendored — upstream ships 5 auxiliary file(s) for this skill (API_WORKFLOWS.md, ERROR_WORKFLOWS.md, NODE_ERROR_OUTPUTS.md, README.md, RESPONSE_SHAPES.md); MIT permits copying, they stay upstream by import policy.
