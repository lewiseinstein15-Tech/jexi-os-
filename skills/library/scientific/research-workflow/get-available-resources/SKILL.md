...
whenToUse: Use when the task matches this skill's scope: Detect host inventory and effective CPU, memory, disk, scheduler, container, and accelerator limits when a user asks for resource-aware planning or before a clearly resource-sensitive local workload. Produces a redacted JSON snapshot and conservative planning helpers without stress tests or assuming visible host hardware is usable.
allowedTools: [Read, Write, Edit, Bash]
domain: research-workflow
tier: reference-only
origin: K-Dense-AI/scientific-agent-skills
upstreamPath: skills/get-available-resources/SKILL.md
...
- Source: K-Dense-AI/scientific-agent-skills `skills/get-available-resources/SKILL.md` @ `330c8e764435` (release 2.69.0); body ported verbatim.
