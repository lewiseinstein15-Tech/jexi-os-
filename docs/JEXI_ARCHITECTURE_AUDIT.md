...
| 8 | Part 13 | "AndroidRuntime" adapter — REAL adb-backed device use (terminal, browser via am start + uiautomator dump, input, screencap, push); the device IS the computer, no host Chromium | **DONE — B225** (`src/services/AndroidRuntime.js`, wired into ComputerRuntime + the browser loop; `test-b225.js` 22 android checks via a stub adb binary: exact argv, XML parse, bounds-center taps, PNG validation, and every honest-absence path — no adb / no device → one honest COMPUTER_BLOCKED, never faked; activation `COMPUTER_RUNTIME=android`, documented in ANDROID.md) |
