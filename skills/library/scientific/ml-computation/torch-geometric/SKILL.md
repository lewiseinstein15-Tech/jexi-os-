...
whenToUse: Use when the task matches this skill's scope: PyTorch Geometric (PyG) for graph neural networks — node/link/graph classification, message passing (GCN, GAT, GraphSAGE, GIN), heterogeneous graphs, neighbor sampling, and custom datasets. Use when working with torch_geometric, not for general NetworkX analytics or non-graph PyTorch models.
allowedTools: [Read, Write, Edit, Bash]
domain: ml-computation
tier: reference-only
origin: K-Dense-AI/scientific-agent-skills
upstreamPath: skills/torch-geometric/SKILL.md
...
- Source: K-Dense-AI/scientific-agent-skills `skills/torch-geometric/SKILL.md` @ `330c8e764435` (release 2.69.0); body ported verbatim.
