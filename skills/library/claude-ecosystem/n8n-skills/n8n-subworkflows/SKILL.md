...
whenToUse: "Build reusable, composable n8n sub-workflows. Use when extracting shared logic, building anything multi-step or reused across workflows, or any workflow over ~10 nodes — and whenever the user mentions sub-workflows, Execute Workflow, reuse, shared/common logic, modular workflows, \"Define Below\" inputs, waitForSubWorkflow, mode each vs all, or exposing a workflow as an agent tool. Covers typed sub-workflow inputs, all-vs-each execution, verb-first naming for discovery, stateless vs stateful design, and splitting by input shape."
allowedTools: []
domain: claude-ecosystem
tier: reference-only
origin: czlonkowski/n8n-skills
upstreamPath: skills/n8n-subworkflows/SKILL.md
...
## Import Provenance
- Source: czlonkowski/n8n-skills `skills/n8n-subworkflows/SKILL.md` @ `19cd793f4789` (plugin release 1.35.0); body ported verbatim.
...
- External files: NOT vendored — upstream ships 3 auxiliary file(s) for this skill (NAMING_AND_DISCOVERY.md, README.md, SUBWORKFLOW_PATTERNS.md); MIT permits copying, they stay upstream by import policy.
