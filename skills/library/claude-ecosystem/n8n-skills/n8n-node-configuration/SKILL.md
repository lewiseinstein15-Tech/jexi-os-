...
whenToUse: "Operation-aware node configuration guidance. Use when configuring nodes, understanding property dependencies, determining required fields, choosing between get_node detail levels, or learning common configuration patterns by node type. Always use this skill when setting up node parameters — it explains which fields are required for each operation, how displayOptions control field visibility, and when to use patchNodeField for surgical edits vs full node updates."
allowedTools: []
domain: claude-ecosystem
tier: reference-only
origin: czlonkowski/n8n-skills
upstreamPath: skills/n8n-node-configuration/SKILL.md
...
## Import Provenance
- Source: czlonkowski/n8n-skills `skills/n8n-node-configuration/SKILL.md` @ `19cd793f4789` (plugin release 1.35.0); body ported verbatim.
...
- External files: NOT vendored — upstream ships 4 auxiliary file(s) for this skill (DEPENDENCIES.md, NODE_FAMILY_GOTCHAS.md, OPERATION_PATTERNS.md, README.md); MIT permits copying, they stay upstream by import policy.
