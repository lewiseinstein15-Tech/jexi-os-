 ...
 *   ├── phases/
 *   │   ├── pre-recon.phase.js     1/6 source analysis
 *   │   ├── recon.phase.js         2/6 live app exploration
 *   │   ├── vulnerability.phase.js 3/6 five parallel OWASP agents
 *   │   ├── exploitation.phase.js  4/6 PoC validation (the DOER'S claims)
 *   │   ├── verification.phase.js  5/6 independent re-execution (Phase 8G)
...
export const PHASE_ORDER = ['pre-recon', 'recon', 'vulnerability', 'exploitation', 'verification', 'reporting'];
export const PHASES = [preRecon.phase, recon.phase, vulnerability.phase, exploitation.phase, verification.phase, reporting.phase];
...
export function createWorkflow({ engagementId, sourceRoot, baseUrl, stateRoot, engagement = null, engagementsStore = null, dualNetwork = null, execBridge = null, verifiedBy = null, exploitedBy = null }) {
  ...
  return new DurableWorkflow({
    ...
    ctx: {
      ...
      // Phase 8(G): agent identities for the verifier ≠ doer rule. Defaults
      // keep the verifier a distinct actor from the exploitation phase.
      verifiedBy: verifiedBy || 'security-verifier',
      exploitedBy: exploitedBy || 'pipeline:exploitation',
    },
